...
for filename in os.listdir(os.getcwd()+'\\archives\\1\\'+sys.argv[1]+'\\'+sys.argv[2]+'\\'+sys.argv[3]):
	if(filename.endswith('.csv')):
		print(filename)
		df=pd.read_csv(os.getcwd()+'\\archives\\1\\'+sys.argv[1]+'\\'+sys.argv[2]+'\\'+sys.argv[3]+'\\'+filename)
		print(df)
		df.plot(kind='scatter', x='Time', y='Cost', title=os.path.splitext(filename)[0])
		plt.savefig(os.getcwd()+'\\archives\\1\\'+sys.argv[1]+'\\'+sys.argv[2]+'\\'+sys.argv[3]+'\\'+os.path.splitext(filename)[0]+'.png')
