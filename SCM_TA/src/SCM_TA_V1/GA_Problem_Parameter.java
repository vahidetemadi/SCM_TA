...
public class GA_Problem_Parameter {
	...
	public static ArrayList<ArrayList<DefaultEdge>> getValidSchedulings(DirectedAcyclicGraph<Bug, DefaultEdge> DAG){
		...
		long num=ArithmeticUtils.factorial(potentilEdges.size());
		System.out.println(num);
		long seed=30000/t;
		long c=0;
		for(ICombinatoricsVector<DefaultEdge> perm:potentialPerm){
			System.out.println("c:"+c +"seed: "+seed);
			if(c%seed==0){
		...
		ArrayList<Edge> shuffledPotentialEdges=(ArrayList<Edge>)potentilEdges.clone();
		long t=1000;
		long seed=30000/t;
		long c=0;
		for(int i=0;i<t;i++){
